...
The phases below describe the **implementation order** we used to build v1.0 — they are not a release plan. All six phases shipped together as the initial `1.0.0` cut. The file-count budgets are retained because they are still useful as **scope-creep signals** for future work: if changes in one area start pushing a phase's src count well past its budget, cut features or defer them.
...
## 9. Total scope at v1.0

Roughly **~95 files** across src + tests + docs + examples. About **~50 files in `src/jellycell/`** across all phases.

This is the right size for one person to hold in their head end-to-end. If one phase's file count creeps past its budget while you're extending it, that's a scope-creep signal to cut back — the phase budgets are the ceiling.
