...
Each phase ends with a tagged release. Files listed are new unless marked `[mod]`.

### Phase 0 — Skeleton

**Goal:** name locked on PyPI, CI green, empty but valid package.

**Size: 15 files.** Release `0.0.1` end of phase.
...
**Size: 13 src files + 8 test/fixture files = 21 files.**
...
**Size: 13 src files + 9 test files = 22 files. Load-bearing phase.**

Release `0.0.2` end of phase.

### Phase 3 — Render

**Goal:** `jellycell render notebooks/foo.py` produces a self-contained HTML page in `reports/`.

**Size: 11 src files + 3 test files = 14 files.**

### Phase 4 — Live viewer

**Goal:** `jellycell view` serves the catalogue.

**Size: 5 src files + 2 test files = 7 files.** Release `0.0.3`.

### Phase 5 — Export

**Goal:** ipynb / MyST markdown export with reattached outputs.

**Size: 4 src files + 3 test files = 7 files.**

### Phase 6 — Agent surface + polish + release

**Goal:** `0.1.0`. Two examples work. README honest. `jellycell prompt` emits the canonical agent guide.

**Size: ~21 files touched.**

---

## 9. Total scope at v0.1.0

Roughly **~95 files** across src + tests + docs + examples. About **~45 files in `src/jellycell/`** after Phase 6.

This is the right size for one person to hold in their head end-to-end. If any phase's file count is creeping past what's listed, it's a scope-creep signal to cut back — the phases are the budget.
