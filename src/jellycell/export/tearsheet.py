...
def _header_lines(
    ...
    being a second heading. Includes a link to the rendered HTML report
    (``reports/<stem>.html``) when one exists on disk — readers who want
    ...
    # Configurable reports path — cheap lookup, skip the link if missing.
    for reports_name in ("reports",):
        html = project_root / reports_name / f"{stem}.html"
        if html.exists():
